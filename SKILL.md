...
```yaml
# docker-compose.yml — auto-generated by Ironman
version: "3.9"
...
services:
  ...
  redis:
    ...
    command: redis-server --requirepass ${REDIS_PASS:-redispass}
    ports:
      - "${REDIS_PORT:-6379}:6379"
    volumes:
      - redisdata:/data
    healthcheck:
      test: ["CMD", "redis-cli", "-a", "${REDIS_PASS:-redispass}", "ping"]
      interval: 10s
      timeout: 5s
    restart: unless-stopped
    networks:
...
```bash
# .env.example — auto-generated, copy to .env dan ubah value
# === App ===
APP_PORT=3000
NODE_ENV=production
SECRET_KEY=CHANGE_ME_GENERATE_RANDOM_64CHAR

# === Database (PostgreSQL) ===
DB_HOST=db
DB_PORT=5432
DB_USER=appuser
DB_PASS=changeme          # ⚠️ WAJIB diganti di production
DB_NAME=appdb
...
REDIS_HOST=redis
REDIS_PORT=6379
REDIS_PASS=redispass       # ⚠️ WAJIB diganti di production

# === Grafana ===
GRAFANA_PORT=3001
GRAFANA_USER=admin
GRAFANA_PASS=admin123      # ⚠️ WAJIB diganti di production
```
...
### 2. Default Ports & Credentials
| Service   | Port  | Username | Password   |
|-----------|-------|----------|------------|
| App       | 3000  | —        | —          |
| Postgres  | 5432  | appuser  | changeme   |
| Redis     | 6379  | —        | redispass  |
| Grafana   | 3001  | admin    | admin123   |

> ⚠️ **Ganti semua default password di .env sebelum deploy ke production!**
...
### 4. Backup Database
 docker exec -t db pg_dumpall -U appuser > backup_$(date +%F).sql

### 5. Restore
 cat backup_2026-03-20.sql | docker exec -i db psql -U appuser
```

### GitHub Backup Script
```bash
#!/bin/bash
# backup-to-github.sh — backup compose project ke GitHub
set -euo pipefail
...
# Dump database
docker exec -t db pg_dumpall -U ${DB_USER:-appuser} > "backups/db_${DATE}.sql"

# Git commit & push
git add -A
git commit -m "backup: ${DATE} — auto-backup via ironman"
git push origin main

echo "✅ Backup pushed to GitHub: ${DATE}"
```

### Compose Patterns Checklist
```
✅ Semua service punya healthcheck
✅ depends_on pakai condition: service_healthy
✅ Volume untuk persistence (jangan ephemeral)
✅ Network isolation (frontend/backend terpisah)
✅ .env.example ada, .env di .gitignore
✅ restart: unless-stopped untuk production
✅ Resource limits (deploy.resources) untuk production
❌ Jangan hardcode password di compose file
❌ Jangan expose port DB ke publik tanpa firewall
```
...
```python
# serve.py — production model serving
...
async def load_model():
    global model
    model = torch.jit.load("models/model_v1.pt")
    model.eval()
...
with mlflow.start_run(run_name="training-v2") as run:
    mlflow.log_params({
        "learning_rate": 0.001,
        "batch_size": 64,
        "epochs": 100,
...
services:
  mlflow:
    ...
    ports:
      - "5000:5000"
    environment:
      BACKEND_STORE_URI: postgresql://mlflow:mlflow@mldb:5432/mlflow
  ...
  mldb:
    ...
    environment:
      ...
      POSTGRES_DB: mlflow
    volumes:
      - mldbdata:/var/lib/postgresql/data
...
class ModelMonitor:
    ...
    def check_data_drift(self, incoming: np.ndarray, threshold: float = 0.05):
        drifted = []
        for i in range(incoming.shape[1]):
            stat, p_value = stats.ks_2samp(self.reference[:, i], incoming[:, i])
            if p_value < threshold:
                drifted.append({"feature": i, "p_value": p_value})
        return drifted

    def check_prediction_drift(self, preds: list, baseline_mean: float):
        z = abs(np.mean(preds) - baseline_mean) / (np.std(preds) / len(preds)**0.5)
        return {"z_score": z, "drifted": z > 3.0}
```
...
jobs:
  train-validate-deploy:
    ...
    steps:
      - uses: actions/checkout@v4
      - name: Train model
        run: python training/pipeline.py --output models/
      - name: Validate model
        run: python scripts/validate_model.py models/model_v1.pt
...
✅ Health endpoint di model API
✅ Data drift monitoring di production
✅ GPU resource limits di K8s/Docker
✅ Model rollback strategy (blue/green atau canary)
✅ CI/CD: auto-train → validate → deploy
